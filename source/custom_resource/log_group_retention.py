...
class LogGroupRetention:
    ...
    def update_retention(self, event):
        ...
        log_groups = cloudwatch.describe_log_groups(logGroupNamePrefix=log_group_prefix)

        lambda_names = self.get_lambda_names(resource_props=event["ResourceProperties"])
    ...
    def set_log_group_retention(
        self, client, log_groups, lambda_names, retention_period
    ):
        for log_group in log_groups["logGroups"]:
            if log_group["logGroupName"] in lambda_names:
                ...
                self.log.info(
                    f'put retention for log group {log_group["logGroupName"]}'
                )
